...
目前不要先研究複雜模型。全隊先完成同一件事：

> 讓官方範例從輸入一路跑到輸出，三個必要檔案都有產生，而且官方四項檢查全部通過。

完成後才依序做：跑完 103 題、建立簡單預測版本、加入文字判讀、改善分數。

看不懂下方技術名稱時，先查[白話版官方規格](OFFICIAL_REQUIREMENTS.md)。
...
交付一個 Docker 預測程式：讀取截止日前的金融數字與文字資料，產生多種可能的未來結果，並且每一題都能穩定完成。
